...
const _sfc_main = {
  ...
  setup(__props, { expose: __expose }) {
    __expose();
    const loanId = common_vendor.ref("");
    ...
    const remindTimeText = common_vendor.computed(() => {
    ...
    function goSchedule() {
      if (loanId.value) {
        common_vendor.index.navigateTo({ url: `/pages/loan/schedule/index?id=${loanId.value}` });
        return;
      }
      if (!(Number(form.principal) > 0 && Number(form.term_months) > 0)) {
    ...
    async function loadDetail() {
      const res = await api_loan.getLoanDetail(loanId.value);
      if (res.success) {
        const data = res.data;
        ...
        form.repayment_method = utils_repayment.normalizeRepaymentMethod(data.repayment_method);
    ...
    async function submitUpdate() {
      ...
      const res = await api_loan.updateLoan(loanId.value, payload);
      if (res.success) {
    ...
    function confirmDelete() {
      common_vendor.index.showModal({
        ...
        success: async ({ confirm }) => {
          if (!confirm)
            return;
          const res = await api_loan.deleteLoan(loanId.value);
          if (res.success) {
    ...
    const __returned__ = { loanId, loaded, showReminderPicker, showFirstPaymentPicker, currentYear, years, months, form, days, hours, minutes, firstPaymentPickerValue, remindPickerValue, methodLabel, monthlyRateText, monthlyPaymentText, remindTimeText, pad2, noop, goSchedule, loadDetail, chooseRepaymentMethod, buildFirstPaymentPickerValue, openFirstPaymentPicker, closeFirstPaymentPicker, onFirstPaymentPickerChange, confirmFirstPaymentPicker, onRemindCardClick, onRemindToggle, openReminderPicker, closeReminderPicker, onRemindPickerChange, confirmReminderPicker, submitUpdate, confirmDelete, computed: common_vendor.computed, reactive: common_vendor.reactive, ref: common_vendor.ref, get onLoad() {
      return common_vendor.onLoad;
    }, get deleteLoan() {
      return api_loan.deleteLoan;
    }, get getLoanDetail() {
      return api_loan.getLoanDetail;
    }, get updateLoan() {
      return api_loan.updateLoan;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return common_vendor.e({
    a: $setup.loaded
  }, $setup.loaded ? common_vendor.e({
    b: $setup.form.loan_name,
    c: common_vendor.o(($event) => $setup.form.loan_name = $event.detail.value),
    d: $setup.form.principal,
    e: common_vendor.o(($event) => $setup.form.principal = $event.detail.value),
    f: common_vendor.t($setup.monthlyRateText),
    g: $setup.form.annual_rate,
    h: common_vendor.o(($event) => $setup.form.annual_rate = $event.detail.value),
    i: $setup.form.term_months,
    j: common_vendor.o(($event) => $setup.form.term_months = $event.detail.value),
    k: common_vendor.t($setup.methodLabel),
    l: common_vendor.o($setup.chooseRepaymentMethod),
    m: common_vendor.t($setup.form.first_payment || "-"),
    ...
    q: common_vendor.t($setup.remindTimeText),
    r: $setup.form.remind_enabled,
    s: common_vendor.o($setup.onRemindToggle),
    t: common_vendor.o($setup.noop),
    v: common_vendor.o($setup.onRemindCardClick),
    w: common_vendor.o($setup.confirmDelete),
    x: common_vendor.o($setup.submitUpdate),
...
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__scopeId", "data-v-64a83418"], ["__file", "/Users/wangjun/Documents/GitHub/金融APP/mobile/src/pages/loan/edit/index.vue"]]);
