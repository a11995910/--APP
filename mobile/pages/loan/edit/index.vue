<template>
  <view class="page" v-if="loaded">
    <scroll-view scroll-y class="content">
      ...
      <view class="section-card">
        ...
        <view class="item-row clickable" @click="openFirstPaymentPicker">
          ...
          <view class="item-value-wrap">
            <text class="item-value">{{ form.first_payment || '-' }}</text>
            <text class="arrow">›</text>
          </view>
        </view>
      ...
      <view class="section-card remind-card" @click="onRemindCardClick">
        <view>
          ...
          <text class="item-sub">{{ remindTimeText }}</text>
        </view>
    ...
    <view class="bottom-actions">
      <button class="action-btn btn-delete" @click="confirmDelete">删除</button>
      <button class="action-btn btn-submit" @click="submitUpdate">提交修改</button>
    </view>
...
/**
 * 贷款编辑页面
 * 职责：加载既有贷款、支持修改/删除、并可跳转查看还款明细。
 */
import { computed, reactive, ref } from 'vue'
import { onLoad } from '@dcloudio/uni-app'
import { deleteLoan, getLoanDetail, updateLoan } from '../../../api/loan'
import {
...
const monthlyPaymentText = computed(() => {
  if (!(Number(form.principal) > 0 && Number(form.term_months) > 0)) return '0.00'
  ...
  return schedule[0]?.payment?.toFixed(2) || '0.00'
})
...
const remindTimeText = computed(() => {
  if (!form.remind_enabled) return '已关闭'
...
/**
 * 页面加载生命周期。
 * @param {Object} options 路由参数，包含贷款ID。
 */
onLoad((options) => {
